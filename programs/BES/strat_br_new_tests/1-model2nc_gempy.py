...
path_output = "../../../output/BES/StartBR/novos_testes/gempy_2.3.1/pickle_model/output/"
...
path_sfp = os.path.join(path_output, "surface_points.csv")
surfpoints.to_csv(path_sfp, index=False)

path_ori = os.path.join(path_output, "orientations.csv")
orientations.to_csv(path_ori, index=False)

path_series = os.path.join(path_output, "series.csv")
series.to_csv(path_series, index=False)

path_surf = os.path.join(path_output, "surfaces.csv")
...
kriging_data_k.insert(0, "Model_ID", "BES")
path_kriging = os.path.join(path_output, "kriging_parameters.csv")
...
rescaling_data_k.insert(0, "Model_ID", "BES")
path_rescale = os.path.join(path_output, "rescaling_parameters.csv")
...
path_res = os.path.join(path_output, "resolution.csv")
...
path_space = os.path.join(path_output, "spacing.csv")
...
path_extent = os.path.join(path_output, "extent.csv")
...
# Create coords
coords = {"Model_ID": model_id, "nx": x_rg, "ny": y_rg, "nz": z_rg}
...
ds["depth"].attrs = {
    "long_name": "depth voxels",
    "unit": "meter",
    "var_desc": "depth is given referenced to unit above sea level",
}
...
ds.to_netcdf(os.path.join(path, f"{model_id}-model.nc"))
...
print(f"Done model id {model_id}, {path_model}")
